...
const AiMessageBubble: React.FC<{ message: ChatMessage }> = ({ message }) => {
    ...
    return (
        <div className={`p-3 max-w-md md:max-w-lg shadow-md bg-gray-700 text-white self-start rounded-lg rounded-bl-none transition-all duration-300`}>
            {message.isSpoken ? (
                <div>
                    <button onClick={() => setIsExpanded(!isExpanded)} className="flex items-center gap-3 w-full text-left">
                        <TutorAvatarIcon mouthShape="X" className="w-8 h-8 flex-shrink-0 bg-gray-800 rounded-full p-1" />
                        <div className="flex-1">
                            <p className="font-semibold">{t('tutor.aiAssistant')}</p>
                            <p className="text-sm text-gray-400">{isExpanded ? t('tutor.hideTranscript') : t('tutor.showTranscript')}</p>
                        </div>
                        {isExpanded ? <ChevronUpIcon className="w-5 h-5 flex-shrink-0"/> : <ChevronDownIcon className="w-5 h-5 flex-shrink-0"/>}
                    </button>
                    {isExpanded && (
                        <p className="mt-3 pt-3 border-t border-gray-600/50 whitespace-pre-wrap animate-fadeIn text-gray-200">{message.text}</p>
                    )}
                </div>
            ) : (
                <p className="whitespace-pre-wrap">{message.text}</p> // Fallback for non-spoken messages
            )}
        </div>
    );
};
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {showStartLearningButton ? (
            <div className="flex flex-col items-center justify-center text-center flex-1">
                ...
                />
                 <div className="mt-4">
                  <p className="text-gray-300 mb-4">{t('tutor.readyToLearn', { topicTitle: currentTopicTitle || 'a new topic'})}</p>
          ...
          <>
            ...
            <div className="flex-1 flex flex-col space-y-4 p-2 overflow-y-auto min-h-0">
                {visibleHistory.map((msg, index) => {
                    if (msg.sender === 'ai') {
                        return <AiMessageBubble key={index} message={msg} />;
                    }
