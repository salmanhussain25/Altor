...
//   return (
//     <div className="bg-gray-800 rounded-2xl flex flex-col h-full shadow-2xl border border-gray-700 overflow-hidden">
...
interface WhiteboardProps {
  ...
  language: string;
  isMuted: boolean;
  onSaveVisualAidCode: (newCode: string) => void;
}
...
export const Whiteboard: React.FC<WhiteboardProps> = ({
  ...
  useEffect(() => {
    if (isFlipped && visualAid) {
        setEditedCode(visualAid.content);
    } else {
        // Reset when flipping back or when visualAid disappears
        setEditedCode(null);
    }
  }, [isFlipped, visualAid]);
  ...
  const handleSave = () => {
    if (editedCode !== null) {
        onSaveVisualAidCode(editedCode);
        // Flip back to see the result immediately
        setIsFlipped(false);
    }
  };

  const handleCopy = () => {
    const codeToCopy = isFlipped ? editedCode : visualAid?.content;
    if (codeToCopy) {
        ...
        {activeTab === 'ANIMATION' && visualAid && !isVisualAidLoading ? (
             <div className="flex items-center gap-2">
                {isFlipped && (
                    <button
                        onClick={handleSave}
                        className="flex items-center gap-2 px-3 py-1.5 bg-green-600 text-white rounded-md text-sm font-semibold hover:bg-green-500 transition-colors"
                        title={t('misc.saveAndRun')}
                    >
                        <CheckCircleIcon className="w-5 h-5"/>
                        <span>{t('misc.saveAndRun')}</span>
                    </button>
                )}
                <button
                    onClick={() => setIsFlipped(!isFlipped)}
